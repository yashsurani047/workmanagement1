...
import { View, Text, TouchableOpacity, Modal, ActivityIndicator, FlatList, StyleSheet, TextInput } from 'react-native';
  ...
  const handleForward = async () => {
    if (selectedUsers.length === 0 || !taskId) {
      Toast.show({
        type: 'error',
        text1: 'Error',
        text2: 'Please select at least one user to forward to',
        position: 'bottom'
      });
      return;
    }
    ...
    try {
      ...
      if (onForwardSuccess) {
        // Call onForwardSuccess for each selected user
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Text style={styles.modalTitle}>Forward Task</Text>
              ...
              {submitting ? (
                <View style={styles.submittingContainer}>
                  ...
                  <Text style={[styles.btnText, { color: theme.colors.white }]}>
                    {selectedUsers.length > 1 ? 'Forwarding to users...' : 'Forwarding...'}
                  </Text>
                </View>
              ) : (
                <Text style={[styles.btnText, { color: theme.colors.white }]}>
                  {selectedUsers.length > 1 
                    ? `Forward to ${selectedUsers.length} users` 
                    : 'Forward Task'}
                </Text>
              )}
            </TouchableOpacity>
